		  ...
		  int half, size_t sz, size_t offset)
{
	...
	memcpy(u.sse, context->fpregs.fpr_xacc + off, sizeof(u.sse));
	     ...
	     size_t sz, size_t offset, enum reg_pool pool, size_t archw)
{
	...
	if (valuep != NULL) {
		...
		long int *reg;
#endif
		reg = (void *)&context->fpregs.fpr_acc[0][0];
...
fetch_stack_pointer(struct fetch_context *context)
{
	arch_addr_t sp;
#ifdef __x86_64__
	sp = (arch_addr_t)context->iregs.r_rsp;
#else
	sp = (arch_addr_t)context->iregs.r_esp;
#endif
	return sp;
}
